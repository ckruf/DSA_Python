...
import pytest
from ch07.my_practice_implementations.favorites_list import FavoritesList, \
  Position, Item
...
class TestMoveUp:
  ...
  def test_move_up_no_move(favorites_list_abcd_cnt_1):
    ...
    """
    original_order = [
      item._element for item in favorites_list_abcd_cnt_1._data
      ]
  ...
  def test_move_up_one_position():
    ...
    new_actual_order = [
      item._element for item in favorites_list._data
    ]
...
class TestTop:
  ...
  def test_fails_on_greater_than_length(self, favorites_list_abcd_cnt_1):
    ...
    with pytest.raises(ValueError) as e_info:
      for elem in \
      favorites_list_abcd_cnt_1.top(len(favorites_list_abcd_cnt_1) + 1):
        pass
  ...
  def test_succeeds_one(self):
    ...
    for item in items:
      favorites_list._data.add_last(item)
    assert [elem for elem in favorites_list.top(1)] == ["A",]
  ...
  def test_succeeds_all(self, favorites_list_abcd_cnt_1):
    ...
    elems = [
      elem for elem in favorites_list_abcd_cnt_1.top(length)
    ]
